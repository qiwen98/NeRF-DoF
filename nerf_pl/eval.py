...
def get_opts():
    ...
    parser.add_argument('--root_dir', type=str,
                        default='D:\WenTemp/nerf_llff_data/fortress',
    ...
    parser.add_argument('--is_bokeh', default=False, action="store_true",
                help='is_bokeh_dataset_need to be set? if this is set, then we will train the model at bokeh dataset after switch_epoch_num' )

    parser.add_argument('--is_LFF', default=False, action="store_true",
                    help='use LFF')
...
if __name__ == "__main__":
    ...
    embedding_xyz = Embedding(3,args.N_emb_xyz,is_LFF=args.is_LFF)
    embedding_dir = Embedding(3,args.N_emb_dir,is_LFF=args.is_LFF)
    embedding_xyz.cuda().eval()
    embedding_dir.cuda().eval()
    # print(next(embedding_xyz.parameters()).is_cuda) # returns a boolean)
    nerf_coarse = NeRF(in_channels_xyz=6*args.N_emb_xyz+3,
                       in_channels_dir=6*args.N_emb_dir+3)
    load_ckpt(nerf_coarse, args.ckpt_path, model_name='nerf_coarse')
    nerf_coarse.cuda().eval()

    # print(next(nerf_coarse.parameters()).is_cuda) # returns a boolean)
